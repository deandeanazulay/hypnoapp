...
export default function ChatInput({
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    // Show cancel if dragged left significantly
    const shouldCancel = deltaX < -60;
    setShowCancelIndicator(shouldCancel);
  };
  ...
  // Global pointer events for drag outside button
  useEffect(() => {
    const handleGlobalMove = (e: PointerEvent) => {
      if (!isDragging) return;
      const deltaX = e.clientX - startPosition.x;
      setDragPosition({ x: deltaX, y: 0 });
      setShowCancelIndicator(deltaX < -60);
    };
    ...
    <div className="fixed left-0 right-0 bg-black/98 backdrop-blur-xl border-t border-white/20 z-50" 
      ...
      {isRecording && (
        <div className="mb-3 bg-gradient-to-br from-red-500/20 to-orange-500/20 backdrop-blur-xl rounded-2xl border border-red-500/30 p-4 animate-bounce-in">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 rounded-full bg-red-500/30 border border-red-500/50 flex items-center justify-center animate-pulse">
                <div className="w-4 h-4 bg-red-500 rounded-full animate-pulse" />
              </div>
              <div>
                <div className="text-white font-medium text-sm flex items-center space-x-3">
                  <span>Recording</span>
                  {/* Waveform Animation */}
                ...
                <div className="text-red-400 text-sm font-mono">{formatTime(recordingDuration)}</div>
              </div>
            </div>
          ...
          <div className="mt-3 text-center">
            <p className="text-white/70 text-xs">
              Hold to record • Slide ← to cancel • Release to stop
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-gradient-to-br from-white/10 to-white/15 backdrop-blur-xl rounded-2xl border border-white/25 p-3 shadow-2xl">
        <form onSubmit={onSubmit} className="flex items-center gap-3">
          ...
          <div className="flex items-center space-x-2">
            {/* Clear Chat Button */}
            {hasMessages && !isRecording && !hasRecording && !inputText.trim() && (
              <button
                type="button"
                onClick={onClearChat}
            ...
            {!inputText.trim() && !hasRecording && (
              <button
                ...
                disabled={isLoading}
                className={`relative p-3 rounded-xl transition-all border select-none touch-none ${
                  isRecording
            ...
            <button
              ...
              className="p-3 bg-gradient-to-r from-teal-400 to-cyan-400 rounded-xl text-black font-semibold hover:scale-110 transition-all disabled:opacity-50 disabled:hover:scale-100 shadow-lg shadow-teal-400/25 flex-shrink-0"
            >
              <Send size={18} />
            </button>
          </div>
