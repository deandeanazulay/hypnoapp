        ...
        <div className="flex-1 min-h-0 px-6">
          <div className="grid grid-cols-2 gap-2 h-full overflow-y-auto">
            ...
            <div className="bg-white/5 backdrop-blur-md rounded-lg p-3 border border-white/10">
              <label className="block text-white/80 text-xs font-medium mb-2">Protocol Name</label>
              <input
                type="text"
                value={protocol.name || ''}
                onChange={(e) => setProtocol(prev => ({ ...prev, name: e.target.value }))}
                placeholder="My Confidence Builder"
                className="w-full bg-white/10 border border-white/20 rounded-lg px-2 py-1.5 text-white placeholder-white/50 focus:outline-none focus:border-teal-400/50 text-xs"
              />
            </div>

            {/* Duration */}
            <div className="bg-white/5 backdrop-blur-md rounded-lg p-3 border border-white/10">
              <label className="block text-white/80 text-xs font-medium mb-2">Duration</label>
              <input
                type="range"
                min="5"
                max="30"
            ...
            {/* Induction Selection */}
            <div className="bg-white/5 backdrop-blur-md rounded-lg p-3 border border-white/10">
              <label className="block text-white/80 text-xs font-medium mb-2">Induction</label>
            ...
            {/* Deepener Selection */}
            <div className="bg-white/5 backdrop-blur-md rounded-lg p-3 border border-white/10">
              <label className="block text-white/80 text-xs font-medium mb-2">Deepener</label>
              <div className="space-y-1">
            ...
            {/* Goals */}
            <div className="bg-white/5 backdrop-blur-md rounded-lg p-3 border border-white/10">
              <label className="block text-white/80 text-xs font-medium mb-2">Goals</label>
              ...
              <div className="flex space-x-1 mb-2">
                <input
                  ...
                  className="flex-1 bg-white/10 border border-white/20 rounded-lg px-2 py-1 text-white placeholder-white/50 focus:outline-none focus:border-teal-400/50 text-xs"
              ...
              <div className="space-y-1 max-h-16 overflow-y-auto">
                {protocol.goals?.slice(0, 3).map((goal, index) => (
                  <div key={index} className="flex items-center justify-between bg-white/5 rounded-lg px-2 py-1">
                    <span className="text-white/80 text-xs truncate">{goal}</span>
                    <button
            ...
            {/* Metaphors */}
            <div className="bg-white/5 backdrop-blur-md rounded-lg p-3 border border-white/10">
              <label className="block text-white/80 text-xs font-medium mb-2">Metaphors</label>
              ...
              <div className="space-y-1 max-h-16 overflow-y-auto">
                {protocol.metaphors?.slice(0, 3).map((metaphor, index) => (
                  <div key={index} className="flex items-center justify-between bg-white/5 rounded-lg px-2 py-1">
                    <span className="text-white/80 text-xs truncate">{metaphor}</span>
                    <button
        ...
        <div className="flex-shrink-0 px-6 py-2">
          <div className="flex space-x-3">
            <button
              onClick={handleSave}
              disabled={!isValid}
              className="flex-1 px-3 py-2 bg-gradient-to-r from-teal-400 to-cyan-400 rounded-lg text-black font-semibold hover:scale-105 transition-transform duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 text-xs"
            >
            ...
            <button
              ...
              disabled={!isValid}
              className="px-3 py-2 bg-white/10 hover:bg-white/20 border border-white/20 rounded-lg text-white font-medium transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-xs"
            >
