...
export default function CreateScreen({ onProtocolCreate, onShowAuth }: CreateScreenProps) {
  ...
  const handleComplete = async () => {
    ...
    const PROTOCOL_COST = 5;
    if ((user.tokens || 0) < PROTOCOL_COST) {
      showToast({ 
        type: 'warning', 
        message: `You need ${PROTOCOL_COST} tokens to create a custom protocol. Complete sessions to earn more tokens!`,
        duration: 5000
      });
      return;
    }
    ...
    try {
      ...
      // Deduct tokens from user profile using GameStateManager
      try {
        await updateUser({ 
          tokens: (user.tokens || 0) - PROTOCOL_COST 
        });
        
        if (import.meta.env.DEV) {
          console.log('[CREATE] Protocol saved and tokens deducted successfully:', data);
        }
              ...
              {user && (user.tokens || 0) < 5 && (
                <div className="bg-gradient-to-br from-red-500/10 to-orange-500/10 rounded-xl p-4 border border-red-500/20">
                  ...
                  <p className="text-white/80 text-sm mb-3">
                    You need 5 tokens to create a custom protocol. You currently have {user.tokens || 0} tokens.
                  </p>
                  <div className="text-white/70 text-sm">
                    <p className="mb-2">Earn tokens by:</p>
                    <ul className="space-y-1 text-xs">
                      <li>• Completing daily sessions (+2 tokens)</li>
                      <li>• Maintaining a 7-day streak (+10 tokens)</li>
                      <li>• Leveling up (+5 tokens)</li>
                    </ul>
                  </div>
                </div>
              )}
