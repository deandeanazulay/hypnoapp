              ...
              <div key={milestone.id} className="flex items-center space-x-6 flex-shrink-0">
                ...
                >
                  ...
                  {isCompleted ? (
                    ...
                    <IconComponent size={24} className={`${isActive ? 'text-orange-400' : 'text-teal-400'}`} />
                  )}
                </button>
...
export default function HomeScreen({ 
  ...
  return (
    <div className="h-full bg-gradient-to-br from-black via-purple-950/20 to-indigo-950/20 relative overflow-hidden">
      ...
      {/* CONTENT WRAPPER — tightened spacing */}
      <div className="relative z-10 h-full flex flex-col items-center justify-start px-4 pt-2 pb-3">
        {/* Tagline - Above orb (tighter line-height & margins) */}
        <div className="text-center mb-1 leading-tight">
          <h2 className="text-white text-lg font-light mb-1 leading-tight">
            Enter with Libero in {currentState.name}
          </h2>
          <p className="text-white/70 text-xs leading-tight">Tap to begin with Libero</p>
        </div>

        {/* Center Orb — no extra margins */}
        <div className="flex-none mt-1 mb-2">
          <Orb 
            onTap={handleOrbTap}
