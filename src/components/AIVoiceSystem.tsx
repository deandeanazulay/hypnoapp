...
export default function AIVoiceSystem({ isActive, sessionType, onStateChange, sessionState, sessionConfig }: AIVoiceSystemProps) {
  ...
  return (
    <div className="fixed bottom-32 left-4 right-4 z-40">
      ...
      {/* Input Controls */}
      <div className="bg-black/95 backdrop-blur-xl rounded-2xl p-4 border border-white/20">
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 pr-12 text-white placeholder-white/50 focus:outline-none focus:border-teal-400/50 focus:bg-white/15 transition-all"
              disabled={isThinking || isListening}
            />
            <button
              type="submit"
              disabled={!textInput.trim() || isThinking}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 w-8 h-8 rounded-lg bg-teal-500/20 hover:bg-teal-500/30 border border-teal-500/40 flex items-center justify-center transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed ${
                  isListening 
