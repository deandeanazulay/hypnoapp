...
export class SessionManager {
  ...
  private async _initializeSession(userContext: any) {
    try {
      ...
      if (this.userId && userContext.customProtocol?.id) {
      ...
      if (savedScript) {
        ...
        // Save the generated script to personal library if user is authenticated
        if (this.userId && this.scriptPlan) {
  ...
  private async _saveScriptToLibrary(userContext: any, script: any) {
    try {
      // Create a protocol entry in personal library with the generated script
      const protocolData = {
        user_id: this.userId,
        ...
        script: script // Save the full generated script
      };
      ...
      if (error) {
        console.error('[SESSION] Error saving script to library:', error);
      } else {
        console.log('[SESSION] Script saved to personal library');
      }
    } catch (error) {
