...
import systemPrompt from '../prompts/sessionScript.system.txt?raw';
import userPromptTemplate from '../prompts/sessionScript.user.template.txt?raw';
...
export async function getSessionScript(params: GetSessionScriptParams): Promise<ScriptPlan> {
  ...
  try {
    const scriptPlan = await geminiCircuitBreaker.execute(async () => {
      // 1. Build user prompt with template substitution
      ...
      // 2. Use Supabase Edge Function instead of direct API call
      ...
      if (!response.ok) {
        ...
        throw new Error(`Script generation API error: ${response.status} ${response.statusText} - ${errorBody}`);
      }
      ...
      try {
        ...
        throw new Error(`Failed to parse script response: ${error.message}. Raw: ${generatedText.substring(0, 200)}`);
      }
